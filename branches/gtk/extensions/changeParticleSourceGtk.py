...
from inksmoto.xmotoTools import createIfAbsent, getExistingImageFullPath
...
class ChangeParticleSource(XmExtGtkElement):
    ...
    def getWidgetsInfos(self):
        return {'particle': ('param', 'type', self.defParticle)}
    ...
    def updateBitmap(self, widget):
        ...
        imgName = xmGuiGtk.bitmapSelectWindow('Particle Source Selection',
                                              AvailableElements()['PARTICLESOURCES']).run()

        if imgName is not None:
            imgFile = AvailableElements()['PARTICLESOURCES'][imgName]['file']
